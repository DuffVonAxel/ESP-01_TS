...
unsigned char sizeString(const unsigned char *texto) // Retorna o tamanho em bytes da 'string' dada.
{
...
unsigned char procString(const unsigned char *s1, const unsigned char *s2) // Retorna a posicao de 's1' dentro de 's2'. Se ='255', nao encontrou.
{
	unsigned char M,N;
...
/* Retorna trecho de dentro de uma string */
void retValorStr(const unsigned char *texOrg, const unsigned char *texIni, unsigned char *texRet,unsigned char vlrQuant,unsigned char vlrOffset) // Retorna trecho de dentro de uma string.
{
	unsigned char x,y,z,conta=0;
	for(x=0;x<20;x++) texRet[x]='\0';		// Limpa var
	x = procString(texIni,texOrg);		// Procura parte de string em outro string.
	x +=vlrOffset;									// Posiciona o ponteiro inicial.
	vlrQuant+=x;
...
//	TXSTA=0x00;				// Limpa registro.
    TXSTAbits.SYNC=0;		// Modo assincrono.
    RCSTAbits.SPEN=1;		// Habilita comunicacao serial.
    TRISCbits.TRISC6=1;		// Config. pino TX (Datasheet pag.239-4550,243-877)
...
void uartTXC(const unsigned char dado)			// Transmite um caracter pela serial.
{
...
void uartTXT(const unsigned char *texto)			// Envia um texto(String) pela serial.
{
...
void lcdIniciar(volatile unsigned char *gate, unsigned char lcdTipo)	// Inicializa o LCD da PCI com botoes: &PORTx, Tipo(lcd16x2, lcd20x4...).
{
...
//	lcdSerFlag=lcdPin;			// LCD pinagem: '0'=Paralelo, '1'=Serial
	...
	*lcdPort = 0x00;							// Limpa o Port
	...
	unsigned int t0;
//	unsigned char t2;
	t0 = (int) gate;						// Puxa endereço físico (PORT)
//	t0 = (int) &gate;						// Puxa endereço físico (PORT)
//	lcdPort = t0;							// Salva o PORT
//	t2 = dhtBitVlr;
//	dhtBit = t2;
//	cfgGate = (t1 + 9);						// Carrega o TRISx (via LAT)
//	cfgGate =(int) (t0 + 18);				// Carrega o TRISx (via PORT)
	cfgGate =(t0 + 18);				// Carrega o TRISx (via PORT)
//	cfgGate = (t0 + (unsigned int)18);				// Carrega o TRISx (via PORT)
//	lcdPortIn=t0;
//	lcdPortOut=(int) (t0 + 9);
	lcdPort=(t0 + 9);
	*cfgGate =(char) 0xC0;							// Configura TRISx
	*lcdPort =(char) 0x00;							// Limpa o Port
#endif
//	if(lcdPin==0)
//	{
...
void lllcdPosicao(const unsigned char lcd01X, const unsigned char lcd01Y)	// Posiciona o cursor: Linha, Coluna.
{
...
void lcdCaracter(const unsigned char lcd01Char, const unsigned char linha, const unsigned char coluna)	// Envia um caracter: Caracter, Linha, Coluna.
{
...
void lcdTexto(const unsigned char *lcd01Texto, const unsigned char linha, const unsigned char coluna)	// Envia uma String: Texto, Linha, Coluna.
{
...
void lcdCustom(unsigned char endCGRAM, unsigned char *lcdDados)	// Caracter do usuario: Endereco(1 a 8), Matriz com os dados.
{
...
#define dhtPino0	bit0(*dhtPortOut,dhtBit)	// LATxBits = 0
	...
	dhtBit = t2;
//	cfgGate = (t1 + 9);						// Carrega o TRISx (via LAT)
	cfgGate =(unsigned int) (t1 + 18);				// Carrega o TRISx (via PORT)
