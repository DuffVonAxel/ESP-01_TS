...
# 88 "mespXX_v04C.c"
const unsigned char espApSsid[] ={"IoT_AP"};
const unsigned char espApPass[] ={"12345678"};
# 98 "mespXX_v04C.c"
const unsigned char espSrvSsid[] ={"ESP_WF"};
const unsigned char espSrvPass[] ={"0123456789"};
const unsigned char espSrvCan[] ={"5"};
const unsigned char espSrvEcn[] ={"3"};
const unsigned char espSrvPorta[]={"333"};




const unsigned char webSrvIp[] ={"192.168.0.100"};
const unsigned char webSrvCanal[]={"80"};
# 118 "mespXX_v04C.c"
const unsigned char dbUser[] ={"iotUserBd"};
const unsigned char dbPass[] ={"iotSENAI127"};
const unsigned char dbAcce[] ={"/enviaBD.php"};
const unsigned char dbTabela[] ={"tabiot"};
const unsigned char dbCpId[] ={"iotId"};
const unsigned char dbCpIn0[] ={"iotin0"};
const unsigned char dbCpIn1[] ={"iotin1"};
# 139 "mespXX_v04C.c"
const unsigned char tSpkDominio[]={"api.thingspeak.com"};
const unsigned char tSpkPorta[] ={"80"};
const unsigned char tSpkKeyWr[] ={"P5PCN67Z5NBNC4D1"};
const unsigned char tSpkKeyRd[] ={"689U0S0KM0QUCIKU"};
const unsigned char tSpkCanalId[]={"1476834"};
...
# 1 "./biblioteca_v18.h" 1



#pragma warning disable 520,1498,1083
# 17 "./biblioteca_v18.h"
void int2Asc(unsigned int valor, unsigned char *buffer,unsigned char digi)
{
 unsigned char vrTI2A;
 if(digi>0) for(vrTI2A=0;vrTI2A<digi;vrTI2A++)buffer[vrTI2A]='\0';
 if(digi>5) digi=5;
 switch(digi)
 {
   case 0:
    break;
   case 1:
    buffer[0]=(valor%10) +0x30;
    break;
   case 2:
    buffer[0]=(valor/10) +0x30;
    buffer[1]=(valor%10) +0x30;
    break;
   case 3:
    buffer[0]=(valor/100) +0x30;
    buffer[1]=((valor/10)%10) +0x30;
    buffer[2]=(valor%10) +0x30;
    break;
   case 4:
    buffer[0]=(valor/1000) +0x30;
    buffer[1]=((valor/100)%10) +0x30;
    buffer[2]=((valor/10)%10) +0x30;
    buffer[3]=(valor%10) +0x30;
    break;
   case 5:
    buffer[0]=(valor/10000) +0x30;
    buffer[1]=((valor/1000)%10)+0x30;
    buffer[2]=((valor/100)%10) +0x30;
    buffer[3]=((valor/10)%10) +0x30;
    buffer[4]=(valor%10) +0x30;
    break;
 }
}


unsigned char hex2Dec(unsigned char valor)
{
    unsigned cont, tst=1, charconv=0;

    for(cont=0;cont<8;cont++)
    {
        if(((unsigned)((valor^cont)>>cont) & 0x01)==1) charconv = charconv + tst;
        tst += tst;
    }
    return (charconv);
}
...
unsigned char sizeString(const unsigned char *texto)
{
...
unsigned char procString(const unsigned char *s1, const unsigned char *s2)
{
 unsigned char M,N;
 M=sizeString(s1);
 N=sizeString(s2);
...
unsigned char asc2Hex(unsigned char *a2hVlr1)
{
 unsigned char a2ha,a2hb,a2hc,a2hd;
 a2hd=sizeString(a2hVlr1);

 switch(a2hd)
 {
  case 3:
   a2ha=(a2hVlr1[0]-'0')*100;
   a2hb=(a2hVlr1[1]-'0')*10;
   a2hc=(a2hVlr1[2]-'0');
   break;
  case 2:
...
void retValorStr(const unsigned char *texOrg, const unsigned char *texIni, unsigned char *texRet,unsigned char vlrQuant,unsigned char vlrOffset)
{
 unsigned char x,y,z,conta=0;
 for(x=0;x<20;x++) texRet[x]='\0';
 x = procString(texIni,texOrg);
 x +=vlrOffset;
 vlrQuant+=x;

 for(z=x;z<vlrQuant;z++)
 {
  texRet[conta]=texOrg[z];
  conta++;
 }
}
# 256 "./biblioteca_v18.h"
...
void uartIniciar(const unsigned char vlrBps)
{
 ...
 if(vlrBps>4)TXSTAbits.BRGH=1;
 else TXSTAbits.BRGH=0;
 SPBRG=matBps[vlrBps];
# 339 "./biblioteca_v18.h"
 TXSTAbits.SYNC=0;
    RCSTAbits.SPEN=1;
    TRISCbits.TRISC6=1;
    TRISCbits.TRISC7=1;
...
# 372 "./biblioteca_v18.h"
void uartTXC(const unsigned char dado)
{
    TXREG = dado;
...
void uartTXT(const unsigned char *texto)
{
    unsigned char i;
    for(i=0;texto[i]!='\0';i++)
    uartTXC(texto[i]);
}
# 532 "./biblioteca_v18.h"
unsigned char *lcdPort;
unsigned const char lcdL1=0x80, lcdL2=0xC0;
unsigned char lcdType=0;
unsigned char lcdSerFlag=0;
# 576 "./biblioteca_v18.h"
void lcdSerial(unsigned char lcd01Valor, unsigned char lcd01Rs)
{




    unsigned char lcd01Mascara, lcd01Flag, lcd01Tmp;
    LATEbits.LATE2 = 0;
    LATEbits.LATE0 = 0;
    LATEbits.LATE1 = lcd01Rs;
    LATEbits.LATE0 = 1;
    _delay((unsigned long)((2)*(20000000/4000000.0)));
    LATEbits.LATE0 = 0;
    lcd01Mascara = 0x08;

    for (lcd01Tmp=0; lcd01Tmp<4; lcd01Tmp++)
    {
  lcd01Flag =(unsigned) lcd01Valor & lcd01Mascara;
  if(lcd01Flag==0) LATEbits.LATE1 = 0;
  else LATEbits.LATE1 = 1;
  LATEbits.LATE0 = 1;
  _delay((unsigned long)((2)*(20000000/4000000.0)));
  LATEbits.LATE0 = 0;
  lcd01Mascara =(unsigned) lcd01Mascara >> 1;
    }
...
void lllcd4Bits(unsigned char valor8Bits, unsigned char valorRS)
{
 extern unsigned char *lcdPort;

    *lcdPort = valor8Bits >> 4;
 if(valorRS==0) *lcdPort &= ~(1<<4);
 if(valorRS==1) *lcdPort |= (1<<4);
...
void lcdSerIniciar(unsigned char *lcdSGate, unsigned char lcdSTipo, unsigned char pinCk, unsigned char pinDt, unsigned char pinEn)
{
 ...
 cfgSerGate = (&t0Ser + 0x80);
 *cfgSerGate=cfgTris;
 *lcdPort = 0x00;
# 663 "./biblioteca_v18.h"
 lcdSerial(0x37,0);
 lcdSerial(0x28,0);
...
void lcdIniciar(volatile unsigned char *gate, unsigned char lcdTipo)
{
 ...
 lcdType=lcdTipo;
# 685 "./biblioteca_v18.h"
 unsigned char *cfgGate;
 unsigned int t0;
...
unsigned char lcdBotao(void)
{
 ...
 if((*lcdPort&(1<<6))) result |= (1<<0);
 if((*lcdPort&(1<<7))) result |= (1<<1);
 return (result);
}

void lllcdPosicao(const unsigned char lcd01X, const unsigned char lcd01Y)
{
 unsigned char lcd01Tmp;
 switch(lcdType)
 {
  case 0:
   if (lcd01X == 1) lcd01Tmp = (unsigned)127 + lcd01Y;
   if (lcd01X == 2) lcd01Tmp = (unsigned)191 + lcd01Y;
   break;
  case 1:
   if (lcd01X == 1) lcd01Tmp = (unsigned)127 + lcd01Y;
   if (lcd01X == 2) lcd01Tmp = (unsigned)191 + lcd01Y;
   if (lcd01X == 3) lcd01Tmp = (unsigned)143 + lcd01Y;
   if (lcd01X == 4) lcd01Tmp = (unsigned)207 + lcd01Y;
   break;
  case 2:
   if (lcd01X < 11) lcd01Tmp = (unsigned)127 + lcd01Y;
   if (lcd01X > 10) lcd01Tmp = (unsigned)192 + lcd01Y;
   break;
  case 3:
   if (lcd01X == 1) lcd01Tmp = (unsigned)127 + lcd01Y;
   if (lcd01X == 2) lcd01Tmp = (unsigned)192 + lcd01Y;
   break;
  case 4:
   if (lcd01X == 1) lcd01Tmp = (unsigned)127 + lcd01Y;
   if (lcd01X == 2) lcd01Tmp = (unsigned)191 + lcd01Y;
   if (lcd01X == 3) lcd01Tmp = (unsigned)147 + lcd01Y;
   if (lcd01X == 4) lcd01Tmp = (unsigned)211 + lcd01Y;
   break;
  case 5:
   if (lcd01X == 1) lcd01Tmp = (unsigned)127 + lcd01Y;
   if (lcd01X == 2) lcd01Tmp = (unsigned)192 + lcd01Y;
 }
    lllcd4Bits(lcd01Tmp,0);
}

void lcdCaracter(const unsigned char lcd01Char, const unsigned char linha, const unsigned char coluna)
{
 lllcdPosicao(linha,coluna);
 lllcd4Bits(lcd01Char,1);
}

void lcdTexto(const unsigned char *lcd01Texto, const unsigned char linha, const unsigned char coluna)
{
 unsigned char colTmpVar;
 colTmpVar=coluna;
    while(*lcd01Texto)
    {
  lllcdPosicao(linha,colTmpVar);
  lllcd4Bits(*lcd01Texto,1);
  lcd01Texto++;
  colTmpVar++;
    }
}

void lcdCustom(unsigned char endCGRAM, unsigned char *lcdDados)
{
...
# 1089 "./biblioteca_v18.h"
static unsigned int *dhtPort;
static unsigned int *dhtPortIn;
static unsigned int *dhtPortOut;
...
void dhtxxIniciar(volatile unsigned char *gate, unsigned char dhtBitVlr)
{
# 1133 "./biblioteca_v18.h"
 ...
 cfgGate =(unsigned int) (t1 + 18);
 dhtPortIn =(unsigned int) t1;
 dhtPortOut=(unsigned int) (t1 + 9);
...
unsigned char dhtxxLer(void)
{
# 1167 "./biblioteca_v18.h"
  unsigned char i, k, dado = 0;
  if(timeOut) return (0);

  for(i = 0; i < 8; i++)
  {
    k = 0;
    while(!((unsigned)((*dhtPortIn^dhtBit)>>dhtBit) & 0x01 & 1))
    {
      k++;
      if(k > 100)
      {
    ...
    if((unsigned)((*dhtPortIn^dhtBit)>>dhtBit) & 0x01 & 1) dado=((dado<<1) | 1);
    else dado=(dado<<1);
...
unsigned char dhtxxChecar(void)
{

  _delay((unsigned long)((40)*(20000000/4000000.0)));
# 1224 "./biblioteca_v18.h"
 unsigned char tst;


 tst=(unsigned)((*dhtPortIn^dhtBit)>>dhtBit) & 0x01;
 if(((unsigned)((*dhtPortIn^dhtBit)>>dhtBit) & 0x01)==0)
 {
  _delay((unsigned long)((80)*(20000000/4000000.0)));
  if(((unsigned)((*dhtPortIn^dhtBit)>>dhtBit) & 0x01)==1)
  {
 ...
 return (0);
# 1254 "./biblioteca_v18.h"
}
# 1268 "./biblioteca_v18.h"
unsigned char dhtxxValor(unsigned char valordht)
{
    ...
    if(dhtxxChecar())
    {
        dnthRegistro.rhInteiro=dhtxxLer();
        dnthRegistro.rhDecimal=dhtxxLer();
        dnthRegistro.tempInteiro=dhtxxLer();
        dnthRegistro.tempDecimal=dhtxxLer();
        dnthRegistro.checksum=dhtxxLer();
    }
...
# 199 "mespXX_v04C.c"
void espClearBuffer(void)
{
 unsigned char cbTmp;
 for(cbTmp=0;cbTmp<160;cbTmp++) espBufferRx[cbTmp]='\0';
 Contador = 0;
}

unsigned char espRetOk(void)
{
 unsigned char str090[]={"OK\r\n"};
 unsigned char varResp;
 varResp=procString(str090,espBufferRx);
 if(varResp<0xFF) varResp=1;
 else varResp=0;
 return(varResp);
}


unsigned char espOperacional(void)
{
 unsigned char str050[]={"AT"};
 unsigned char espVarTmp;
 espClearBuffer();

 uartTXT(str050), uartTXT(atFim);
...
void espReset(void)
{
 ...
 uartTXT(atIni), uartTXT(str040), uartTXT(atFim);
}


void espEcho(unsigned char vlrEcho)
{
 unsigned char str030[]={"ATE0"};
 unsigned char str031[]={"ATE1"};
 espClearBuffer();

 if(vlrEcho==0) uartTXT(str030), uartTXT(atFim);
 if(vlrEcho==1) uartTXT(str031), uartTXT(atFim);
}


void espInfo(unsigned char *vrsAt,unsigned char *vrsSdk,unsigned char *vrsBin)
{
 ...
 uartTXT(atIni), uartTXT(str060), uartTXT(atFim);
 _delay((unsigned long)((1500)*(20000000/4000.0)));
...
void espModoWF(unsigned char vlrModoWF)
{
 unsigned char str070[]={"MODE"};
 espClearBuffer();

 if(vlrModoWF<1) vlrModoWF=1;
 if(vlrModoWF>3) vlrModoWF=3;
 vlrModoWF += '0';
 uartTXT(atIni), uartTXT(atCW0), uartTXT(str070);
 uartTXT(atCur), uartTXC(vlrModoWF), uartTXT(atFim);
}


void espModoCx(unsigned char vlrModoCx)
{
 unsigned char str071[]={"MODE="};

 if(vlrModoCx>1) vlrModoCx=1;
 vlrModoCx += '0';
 uartTXT(atIni), uartTXT(atCIP), uartTXT(str071);
...
void espAPCfg(void)
{
 ...
 uartTXT(atIni), uartTXT(atCW0), uartTXT(str010), uartTXT(atCur);
...
unsigned char espApConectar(void)
{
 ...
 uartTXT(atIni), uartTXT(atCW0), uartTXT(str020);
 uartTXT(str021), uartTXT(atFim);
 _delay((unsigned long)((1000)*(20000000/4000.0)));
 espVarTmp=procString(espApSsid,espBufferRx);

 if(espVarTmp==0xFF)
 {

  uartTXT(atIni), uartTXT(atCW0), uartTXT(str020), uartTXT(atCur);
...
unsigned char espApDesconectar(void)
{
...
void espMux(unsigned char vlrMux)
{
 const unsigned char str081[]={"MUX="};

 espApDesconectar();
 if(vlrMux>1) vlrMux=1;
 if(vlrMux==1) espModoCx(0);
 vlrMux += '0';
 uartTXT(atIni), uartTXT(atCIP), uartTXT(str081), uartTXC(vlrMux);
 uartTXT(atFim);
}

unsigned char lcdIp[16];
...
void espIpMac(void)
{
 ...
 unsigned char eipVlr15[4];
 unsigned char eipTst15[18];
 espClearBuffer();

 uartTXT(atIni), uartTXT(str085), uartTXT(atFim);
 _delay((unsigned long)((1000)*(20000000/4000.0)));


 retValorStr(espBufferRx,str086,eipTst15,15,3);
 for(y=0;y<4;y++)eipVlr15[y]='\0';
 y=0;
 while(p>0)
 {
  do
  {
   eipVlr15[y]=eipTst15[x];
   if(eipTst15[x]=='\0')eipTst15[x]='0';
   if(eipTst15[x]>='0' & eipTst15[x]<='9') lcdIp[x]=eipTst15[x];
   x++;
   y++;
  }while(eipTst15[x]>='0' & eipTst15[x]<='9');
  if(p>1) lcdIp[x]='.';
 ...
 retValorStr(espBufferRx,str087,eipTst15,17,3);
 x=0;
 y=0;
 p=0;
 ...
 for(p=0;p<6;p++)
 {
  ...
  lcdMac[c+1]=y;
  if(p<5)lcdMac[c+2]=':';
  if(x>0x60 && x<0x67)x-=0x57;
  if(x>0x41 && x<0x47)x-=0x37;
  if(x>='0' && x<='9')x-='0';
  if(y>0x60 && y<0x67)y-=0x57;
  if(y>0x41 && y<0x47)y-=0x37;
  if(y>='0' && y<='9')y-='0';
...
void espServidor(unsigned char vlrSrv, unsigned int vlrPorta)
{
 ...
 int2Asc(vlrPorta,vlrPortaAsc,4);



 if(vlrSrv>1) vlrSrv=1;
 vlrSrv += '0';
 ...
 if(vlrSrv=='1')
 {
  ...
  espModoWF(2);
  espMux(1);
  uartTXT(atIni), uartTXT(atCIP), uartTXT(str082);
...
void espListaWiFi(void)
{
 ...
 uartTXT(atIni), uartTXT(atCW0), uartTXT(str091), uartTXT(atFim);
}


void __attribute__((picinterrupt(""))) espInt()
{
...
unsigned char espWebTx(void)
{
 ...
 uartTXT(atIni), uartTXT(atCIP), uartTXT(str001), uartTXT(atAs1);
 ...
 if(varTeste)
 {
  ...
  uartTXT(atIni), uartTXT(atCIP), uartTXT(str005),uartTXT(atFim);
  _delay((unsigned long)((2000)*(20000000/4000.0)));
  varTeste=espRetOk();
  if(varTeste)
  {
   lcdTexto(str006,1,1);
   uartTXT(thSpkL1), uartTXT(thSpkL2), uartTXT(thSpkL3);
  ...
  else lcdTexto(str007,1,1);
 }
 else lcdTexto(str008,1,1);
 return(varTeste);
}

void espNTP(unsigned char *vlrDia, unsigned char *vlrMes, unsigned char *vlrAno, unsigned char *vlrHora, unsigned char *vlrMin)
{
 ...
 const unsigned char srv1[]={"0.br.pool.ntp.org"};
 const unsigned char srv2[]={"1.br.pool.ntp.org"};
 const unsigned char srv3[]={"2.br.pool.ntp.org"};
 ...
 uartTXT(atIni), uartTXT(atCIP), uartTXT(str002),uartTXT(atFim);
 espClearBuffer();
 _delay((unsigned long)((2000)*(20000000/4000.0)));
 retValorStr(espBufferRx,str003,dateVarASC,24,2);
 ...
 {
  case 'J':
   if(dateVarASC[5]=='a') vlrMes[0]='0',vlrMes[1]='1';
   else if(dateVarASC[6]=='n') vlrMes[0]='0',vlrMes[1]='6';
   else vlrMes[0]='0',vlrMes[1]='7';
   break;
  case 'F':
   vlrMes[0]='0',vlrMes[1]='2';
   break;
  case 'M':
   if(dateVarASC[6]=='r') vlrMes[0]='0',vlrMes[1]='3';
   else vlrMes[0]='0',vlrMes[1]='5';
   break;
  case 'A':
   if(dateVarASC[6]=='r') vlrMes[0]='0',vlrMes[1]='4';
   else vlrMes[0]='0',vlrMes[1]='8';
   break;
  case 'S':
  ...
  case 'D':
   ...
   break;
  default:
   vlrMes[0]='_',vlrMes[1]='_';
   break;
 }



 vlrHora[0]='_',vlrHora[1]='_';
 vlrMin[0]='_',vlrMin[1]='_';
 vlrDia[0]='_',vlrDia[1]='_';
 vlrAno[0]='_',vlrAno[1]='_';
 if(dateVarASC[11]!='\0')vlrHora[0]=dateVarASC[11],vlrHora[1]=dateVarASC[12];
 if(dateVarASC[14]!='\0')vlrMin[0]=dateVarASC[14],vlrMin[1]=dateVarASC[15];
 if(dateVarASC[8]!='\0')vlrDia[0]=dateVarASC[8], vlrDia[1]=dateVarASC[9];
 if(dateVarASC[22]!='\0') vlrAno[0]=dateVarASC[22],vlrAno[1]=dateVarASC[23];
}
...
void espDataWeb(unsigned char *vlrDia, unsigned char *vlrMes, unsigned char *vlrAno, unsigned char *vlrHora, unsigned char *vlrMin)
{
 ...
 retValorStr(espBufferRx,str130,dateASC,34,6);


 switch(dateASC[14])
 {
  case 'J':
   if(dateASC[15]=='a') vlrMes[0]='0',vlrMes[1]='1';
   else if(dateASC[16]=='n') vlrMes[0]='0',vlrMes[1]='6';
   else vlrMes[0]='0',vlrMes[1]='7';
   break;
  case 'F':
   vlrMes[0]='0',vlrMes[1]='2';
   break;
  case 'M':
   if(dateASC[16]=='r') vlrMes[0]='0',vlrMes[1]='3';
   else vlrMes[0]='0',vlrMes[1]='5';
   break;
  case 'A':
   if(dateASC[16]=='r') vlrMes[0]='0',vlrMes[1]='4';
   else vlrMes[0]='0',vlrMes[1]='8';
   break;
  case 'S':
   vlrMes[0]='0',vlrMes[1]='9';
 ...
 vlrDia[0]='_',vlrDia[1]='_';
 vlrAno[0]='_',vlrAno[1]='_';
 if(dateASC[23]!='\0') vlrHora[0]=dateASC[23], vlrHora[1]=dateASC[24];
 if(dateASC[26]!='\0') vlrMin[0] =dateASC[26], vlrMin[1] =dateASC[27];
 if(dateASC[11]!='\0') vlrDia[0] =dateASC[11], vlrDia[1] =dateASC[12];
 if(dateASC[20]!='\0') vlrAno[0] =dateASC[20], vlrAno[1] =dateASC[21];
}
...
void subInic(void)
{
 ...
 lcdTexto(str102,1,1);
 for(varTeste=0;varTeste<10;varTeste++)
 {
  lcdCaracter(str103,1,(10+varTeste));
...
void subWifi(void)
{
 ...
 unsigned char varTeste;
 lcdTexto(str104,1,1);
 varTeste=espOperacional();

 if(varTeste)lcdTexto(str105,1,5);
 else lcdTexto(str106,1,5);
}
...
void subHub(void)
{
 ...
 unsigned char varTeste;
 lcdTexto(str107,1,1);
 varTeste=espApConectar();

 if(varTeste)lcdTexto(str108,1,5);
 else lcdTexto(str109,1,5);
}
...
void subInfo(void)
{
 unsigned char vAt[]={"0.0.0.0"};
 unsigned char vSdk[]={"0.0.0"};
 unsigned char vBin[]={"Xxxxx 00"};
 unsigned char str140[]={"AT ="};
 unsigned char str141[]={"SDK="};
 unsigned char str142[]={"BIN="};
 unsigned char str143[]={"Versao Firmware:    "};
 espInfo(vAt,vSdk,vBin);
 lcdTexto(str143,1,1);
 lcdTexto(str140,2,1),lcdTexto(vAt,2,5);
 lcdTexto(str141,3,1),lcdTexto(vSdk,3,5);
 lcdTexto(str142,4,1),lcdTexto(vBin,4,5);
}

void subNtp(void)
{
 unsigned char dia[]={"__"};
 unsigned char mes[]={"__"};
 unsigned char ano[]={"__"};
 unsigned char hora[]={"__"};
 unsigned char min[]={"__"};

 espNTP(dia,mes,ano,hora,min);
 lcdTexto("NTP:                ",3,1);
...
void main(void)
{
 ...
 unsigned char str112[]={"00.0%H   00.0\xDF C"};
 str112[13]=0xDF;
 lcdTexto(str112,4,1);
